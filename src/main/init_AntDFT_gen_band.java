 ...
 * @author agung
 */
public class init_AntDFT_gen_band {

    /**
     * @param args the command line arguments
     */
    public parameter main(int ik, double campur, parameter p_d, init_data init) throws InterruptedException, IOException, ClassNotFoundException {
        Date date = new Date();
        ...
        for (int i = 0; i < param.banyak_atom; i++) {
            r_up.main(param.upf_data.get(atom[i]));
        ...
        new driver_atomic_rho().main_band(param);
        param.awal_panjang = ((int) param.tot_muatan) / 2;// settingan solusi awal, aggar eigen tidak dipengaruhi iband
